window.onload = function() {
  ...
  fetch('data/signs.json').then(r=>r.json()).then(data=>{
    signs=data; populateCategoryFilter(); displaySigns(signs);
  });

  fetch('data/funfacts.json').then(r=>r.json()).then(data=>{
    funFacts=data; displayFunFact();
  });

  fetch('data/teachers.json').then(r=>r.json()).then(data=>{
    teachers=data; displayTeachers();
  });
  ...
  function filterSigns(){
    ...
    const filtered=signs.filter(s=>s.palabra.toLowerCase().includes(term) && (categoryFilter.value==='all'||s.categoria===categoryFilter.value));
  ...
  function displayTeachers(){
    ...
    teachers.forEach(t=>{
      ...
      const img=document.createElement('img'); img.src=t.foto; img.alt=t.nombre; card.appendChild(img);
      const name=document.createElement('h3'); name.textContent=t.nombre; card.appendChild(name);
      const years=document.createElement('p'); years.textContent=`Años enseñando LSA: ${t.anosEnsenando}`; card.appendChild(years);
      const desc=document.createElement('p'); desc.textContent=t.descripcion; card.appendChild(desc);
